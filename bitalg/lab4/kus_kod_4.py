...
class AVLTree:
    ...
    def slow_search(self, LineA, LineB):
        nodes = []
        returned = []
        self.get_all(self.root, nodes)
        for node in nodes:
            if (node.start,node.end) == LineA or (node.start,node.end) == LineB:
    ...
    def swap_and_get_neighbours(self, lineA, lineB):
        nb = {}
        nodes = self.slow_search(lineA,lineB)
...
def fix_tree(tree):
    a = []
    tree.get_all(tree.root, a)
    new_tree = AVLTree()
    new_tree.x = tree.x
    lineset = set([(node.start,node.end) for node in a])
    for line in lineset:
        if line[1][0] > tree.x:
            new_tree.insert_line_segment(line[0],line[1])
    return new_tree
...
def give_segments():
    max_lines = input("Give the amount of segments: ")
    line_segments = []
    fig, ax = plt.subplots()
    ax.set_xlim(0, 800)
    ax.set_ylim(0, 600)
    ax.set_title(f"Draw {max_lines} Line Segments")
    start_pos = None
    def on_click(event):
        nonlocal start_pos
        if event.inaxes != ax:
            return
        if len(line_segments) >= max_lines:
            print("Max line segments reached.")
            return
        if start_pos is None:
            start_pos = (event.xdata, event.ydata)
        else:
            end_pos = (event.xdata, event.ydata)
            line_segments.append((start_pos[0], start_pos[1], end_pos[0], end_pos[1]))
            ax.plot([start_pos[0], end_pos[0]], [start_pos[1], end_pos[1]], color="black", lw=2)
            start_pos = None
        plt.draw()
...
segments = [((-7.32,-2.22),(0.74,4.56)),((-6.86,2.06),(3.02,-4.54)),((-6.96,0.18),(-5.8,0.18)),((-3.36,0.44),(-2.38,0.38)),((-2.24,-3.06),(-1.52,4.14)),((-0.84,-3.8),(-0.1,4.72)),((-0.84,-3.8),(-0.1,4.72)),((-0.06,0.46),(1.7,0.08))]
a = find_intersections(segments)
print(len(a))
#b = find_intersections(give_segments())
#print(len(b))